...
describe('scoring', () => {
  ...
  it('does not count beyond the supplied fetched-data horizon', () => { const e=[event(),event({timestamp:new Date('2026-01-06T12:00:00Z')}),event({timestamp:new Date('2026-01-03T00:00:00Z')})]; const estimate=estimateAnonymitySet(e,'STRK',1000000000000000000n,86400000); expect(estimate.within24h).toBe(2); expect(estimate.within7d).toBe(2); });
